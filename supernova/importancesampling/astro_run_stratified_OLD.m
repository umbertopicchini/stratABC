...
numattempts = 2000;
pilot_numresample = numresample1;  % just for the purpose of producing some distances on resampled data
...
%        index_inclusion1 = distance < 1.5*ABCthreshold;
       index_inclusion1 = distance < 0.28;
       n1 = sum(index_inclusion1);
       omega1 = n1/numresample1; 
%        index_inclusion2 = (distance < 2*ABCthreshold) & ~(distance < 1.5*ABCthreshold);
       index_inclusion2 = (distance < 0.30) & ~(distance < 0.28);
       ...
       index_inclusion3 =  ~(distance < 0.30);
       ...
       % I could have imposted automatic rejection if n1test==0 or n2test==0 or n3test==0
       % however this is in practice not necessary: for example, say that
       % n1test==0, then certainly index_inclusion1 is empty and hence
       % distance1 is an empty vector. For an empty input, logsumexp()
       % returns -Inf and hence the proposal will certainly be rejected
