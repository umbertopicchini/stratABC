function sim_summ = astro_simsummaries(bigtheta,nobs,nbin)

% simulate n x numsim "data points" iid from a Gaussian with mean 0.5 and standard deviation 0.05,
% truncated to the interval [0.01,1.2].
uniform_draws = rand(nobs,1);
zdata = mytruncgaussrandraw([0.01,1.2,0.5,0.05],uniform_draws);
...
[~,zcentre]= hist(zdata,nbin);  % bin the data and collect the centres of the bins

sim_summ = zeros(nbin,1);       % this will collect the simulated "summaries"
for ii=1:nbin
     sim_summ(ii) = distmodulus(zcentre(ii),om,ok,ol,w0,wa,h0);
end
