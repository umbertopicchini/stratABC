function ABCMCMC = rsabcmcmc(problem,data,bigtheta,sampletime,stoichiometry,parmask,parbase,numresample1,numresample2,R_mcmc,step_rw,burnin,length_CoVupdate,covar,ABCthreshold)
...
% create four "block of indeces" as in R. Everitt "Bootstrapped synthetic likelihood";
block1 = [1:nobs/4]';
...
% resampling indices (to compute the strata probabilities
    ...
    for jj=1:numresample1
       ...
       for ii=1:length(indeces)
         if indeces(ii) == 1
            resample_indeces1(count+1:count+8,jj) =  block1;
         elseif indeces(ii) == 2
            resample_indeces1(count+1:count+8,jj) =  block2; 
         elseif indeces(ii) == 3
            resample_indeces1(count+1:count+8,jj) =  block3;
         else
            resample_indeces1(count+1:count+8,jj) =  block4; 
         end
         count = count + 8;
       end
    end
   ...
   for jj=1:numresample2
     ...
     for ii=1:length(indeces)
         if indeces(ii) == 1
            resample_indeces2(count+1:count+8,jj) =  block1;
         elseif indeces(ii) == 2
            resample_indeces2(count+1:count+8,jj) =  block2; 
         elseif indeces(ii) == 3
            resample_indeces2(count+1:count+8,jj) =  block3;
         else
            resample_indeces2(count+1:count+8,jj) =  block4; 
         end
         count = count + 8;
     end
   end
...
for mcmc_iter = 1:R_mcmc
   ...
   simdata2= feval([problem, '_model'],bigtheta,sampletime,stoichiometry,1); 
    % resamples simulated data    
    count = 0;
    for jj=1:numresample2
       simdata_resampled2(:,jj+count:jj+1+count) = simdata2(resample_indeces2(:,jj),:);
       count = count +1;
    end
   ...
   index_inclusion1 = distance < 0.5*ABCthreshold;
   n1test = sum(index_inclusion1); % number ofsummaries falling into the ellipsis above
   ...
   if n1test==0 || n2test==0 || n3test==0 % skip the rest of the loop because of neglected strata
    ...
    simdata1= feval([problem, '_model'],bigtheta,sampletime,stoichiometry,1); 
    % resamples simulated data    
    count = 0;
    for jj=1:numresample1
       simdata_resampled1(:,jj+count:jj+1+count) = simdata1(resample_indeces1(:,jj),:);
       count = count +1;
    end
    ...
    distance = sqrt(sum((xc / covar) .* xc, 2));
    index_inclusion1 = distance < 0.5*ABCthreshold;  % was 5.11 and 6.55
