...
for mcmc_iter = 1:R_mcmc
    ...
    % resamples simulated training data using the overlapping blocks bootstrap with size 8 
     count = 0;
    ...
    index_inclusion1 = distance < 3*ABCthreshold;  % was 2.5 and 5
    n1 = sum(index_inclusion1);
    omega1 = n1/numresample1; 
    index_inclusion2 = (distance < 5*ABCthreshold) & ~(distance < 3*ABCthreshold);
   ...
   index_inclusion3 =  ~(distance < 5*ABCthreshold);
