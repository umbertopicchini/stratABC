...
prior_pred_numresample = 1000;  % it should not be larger than 256 as it would be pointless. There are only 256 different ways to choose 4 blocks with repetitions
 ...
 %  xhat = lv_model(bigtheta_pilot,sampletime,stoichiometry,1);
   ...
   try
    ...
    for jj=1:prior_pred_numresample 
        test = stationaryBB(xhat,2,[4 8]);
       simdata_resampled(:,jj+count:jj+1+count) = test;
       count = count +1;
    end
