function ABCMCMC = rsabcmcmc_statboot(problem,datadiff,bigtheta,sampletime,stoichiometry,parmask,parbase,numresample1,numresample2,R_mcmc,step_rw,burnin,length_CoVupdate,covar,ABCthreshold)
...
summobs = feval([problem, '_abc_summaries'],datadiff); % vector of summaries for the observed data
...
for mcmc_iter = 1:R_mcmc
    ...
    simdata1= feval([problem, '_model'],bigtheta,sampletime,stoichiometry,1); 
     count = 0;
     simdata1diff = diff(simdata1,2); % order-two differences for enhanced stationarity
     for jj=1:numresample1
         % resamples simulated training data using the stationary bootstrap
         % with blocks of random size picked uniformly between 4 and 8
        simdata_resampled1(:,jj+count:jj+1+count) = stationaryBB(simdata1diff,2,[4;8]);
        count = count +1;
     end
    ...
    index_inclusion1 = distance < 4;  % was 2.5 and 5
    n1 = sum(index_inclusion1);
    omega1 = n1/numresample1; 
    index_inclusion2 = (distance < 8) & ~(distance < 4);
   ...
   simdata2= feval([problem, '_model'],bigtheta,sampletime,stoichiometry,1);      
    count = 0;
    simdata2diff = diff(simdata2,2); % order-two differences for enhanced stationarity
    for jj=1:numresample2
         % resamples simulated testing data using the stationary bootstrap
         % with blocks of random size picked uniformly between 4 and 8
       simdata_resampled2(:,jj+count:jj+1+count) = stationaryBB(simdata2diff,2,[4;8]);
       count = count +1;
    end
