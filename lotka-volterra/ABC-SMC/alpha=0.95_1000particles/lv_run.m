...
% simulate distances, to have an idea of the initial threshold to take

abc_distances = zeros(size(summaries_pilot_fixed,1),1);
for ii=1:size(summaries_pilot_fixed,1)
    sim_summaries = summaries_pilot_fixed(ii,:);
    xc = bsxfun(@minus,sim_summaries,summobs');
...
% ABC settings
numparticles = 1000;
ABCthreshold = prctile(abc_distances,50);  % a starting value found by trial and error. Once ABC-SMC is started, the threshold will be automatically updated
alpha = 0.8;  % % how fast do we let the ABC threshold decrease


tic
[ABCdraws, weights] = abc_smc(problem,yobs,bigtheta_start,sampletime,stoichiometry,parmask,parbase,summ_weights,ABCthreshold,numparticles,alpha);
eval = toc
