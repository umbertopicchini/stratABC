function xhat = lv_model(bigtheta,simultime,sampletime,stoichiometry,numsim)
...
try
  % [t,x] = directMethod(stoichiometry', prop, [sampletime(1),sampletime(end)], xinit', const_rates);
    x = SSAv(simultime, const_rates,prop,stoichiometry, xinit, 1 );
catch
...
% extract all simulations for the second coordinate
  ...
  x1=interp1(simultime,x1,sampletime);
  x2=interp1(simultime,x2,sampletime);
  xhat = [x1',x2'];
catch
